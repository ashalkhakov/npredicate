...
public class Test
{
    ...
    public void TestAnd()
    {
        ...
        // (YES AND NO)
        Assert.False(CompoundPredicate.And([yes, no]).EvaluateObject<object>(null));
        // (YES AND YES)
        Assert.True(CompoundPredicate.And([yes, yes]).EvaluateObject<object>(null));
        // (NO AND NO) 
        Assert.False(CompoundPredicate.And([no, no]).EvaluateObject<object>(null));
        // (YES AND YES AND YES AND YES AND YES)
        Assert.True(CompoundPredicate.And([yes, yes, yes, yes, yes]).EvaluateObject<object>(null));
    }
    ...
    public void TestLinq()
    {
        ...
        Predicate or = CompoundPredicate.Or([yes, no]);
        ...
        var v = l.Where(or.LinqExpression<bool>().Compile());
    ...
    public void TestIn()
    {
        // SELF IN { 1, 2, 3, 4 }
        var a = Expr.MakeEvaluatedObject();
        var b = Expr.MakeConstant(new List<int>([1, 2, 3, 4]));
    ...
    public void TestBetween()
    {
        // SELF BETWEEN { 1, 4 }
        var a = Expr.MakeEvaluatedObject();
        var b = Expr.MakeConstant(new List<int>([1, 4]));
    ...
    public void TestAggregate()
    {
        ...
        var agg = Expr.MakeAggregate([zero, one]);
    ...
    public void TestSum()
    {
        var l = Expr.MakeAggregate([Expr.MakeConstant(1), Expr.MakeConstant(2), Expr.MakeConstant(3),]);
        var sum = Expr.MakeFunction("sum:", [l]);
